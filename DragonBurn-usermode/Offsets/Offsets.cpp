...
void Offsets::SetOffsets(const std::string& offsetsData, const std::string& buttonsData, const std::string& client_dllData)
{
    ...
    this->Pawn.EnemySensor = 0;
    if (client_dllJson.contains("C_BaseEntity") && client_dllJson["C_BaseEntity"]["fields"].contains("m_flDetectedByEnemySensorTime"))
    {
        this->Pawn.EnemySensor = client_dllJson["C_BaseEntity"]["fields"]["m_flDetectedByEnemySensorTime"];
    }
    if (this->Pawn.EnemySensor == 0 && client_dllJson.contains("C_CSPlayerPawnBase") && client_dllJson["C_CSPlayerPawnBase"]["fields"].contains("m_flDetectedByEnemySensorTime"))
    {
        this->Pawn.EnemySensor = client_dllJson["C_CSPlayerPawnBase"]["fields"]["m_flDetectedByEnemySensorTime"];
    }
    if (this->Pawn.EnemySensor == 0 && client_dllJson.contains("C_CSPlayerPawn") && client_dllJson["C_CSPlayerPawn"]["fields"].contains("m_flDetectedByEnemySensorTime"))
    {
        this->Pawn.EnemySensor = client_dllJson["C_CSPlayerPawn"]["fields"]["m_flDetectedByEnemySensorTime"];
    }
...
void Offsets::UpdateOffsets()
{
    ...
    bool okLocal =
        ReadFileText("output/offsets.json", offsetsData) &&
        ReadFileText("output/buttons.json", buttonsData) &&
        ReadFileText("output/client_dll.json", client_dllData);
