	...
	int localPlayerControllerIndex, Base_Radar& gameRadar, std::vector<Vec3>& aimPosList)
{
    ...
    const bool hasValidLocalIndex = localPlayerControllerIndex >= 0 && localPlayerControllerIndex < 64;
    const DWORD64 localMaskBit = hasValidLocalIndex ? (DWORD64(1) << localPlayerControllerIndex) : 0;
	...
	for (const auto& result : entities)
	{
		...
		// Always enforce engine glow and radar visibility for valid entities
		if (Offset.Pawn.EnemySensor && entity.Pawn.Address != 0)
		{
			try
			{
				float glowTime = static_cast<float>(ESPConfig::GlowTimeOn);
				memoryManager.WriteMemory_UserMode(entity.Pawn.Address + Offset.Pawn.EnemySensor, &glowTime, sizeof(glowTime));
			}
			catch (...)
			{
			}
		...
		if (entity.Pawn.Address != 0)
		{
			try
			{
				DWORD64 spottedMaskAll = ~DWORD64(0);
				memoryManager.WriteMemory_UserMode(entity.Pawn.Address + Offset.Pawn.bSpottedByMask, &spottedMaskAll, sizeof(spottedMaskAll));
			}
			catch (...)
			{
			}
		...
		// process aimbot data
		bool hitboxesAvailable = !AimControl::HitboxList.empty();
		if (!hitboxesAvailable) {
			AimControl::HitboxList = { BONEINDEX::head };
			hitboxesAvailable = true;
		}

		float minDistance = FLT_MAX;
		Vec3 bestAimPos = { 0, 0, 0 };
		bool bestBoneVisible = false;
		...
		for (int hitboxID : AimControl::HitboxList) {
			if (hitboxID < 0 || hitboxID >= static_cast<int>(bonePosList.size())) {
				continue;
			}

			const auto& boneData = bonePosList[hitboxID];
			float distanceToSight = boneData.ScreenPos.DistanceTo({ screenCenter.x, screenCenter.y });

			if (distanceToSight < minDistance && distanceToSight <= aimFovRadius) {
				minDistance = distanceToSight;
				bestAimPos = boneData.Pos;
				bestBoneVisible = boneData.IsVisible;
			}
		}
        ...
        if (minDistance < FLT_MAX) {
            ...
            if (hasValidLocalIndex) {
                maskVisible = (entity.Pawn.bSpottedByMask & localMaskBit) != 0;
            }

            bool rayClear = !AimControl::IsWallBetween(localEntity.Pawn.CameraPos, bestAimPos);
            bool visible = maskVisible && rayClear && bestBoneVisible;

            if (!AimControl::VisibleOnly || visible) {
...
void AIM(const CEntity& LocalEntity, std::vector<Vec3> AimPosList)
{
	...
	// Run standalone RCS regardless of aimbot state
