			...
			ImGui::BeginChild("Page", MenuConfig::WCS.ChildSize, false, ImGuiWindowFlags_NoScrollbar);
			{
				...
				if (MenuConfig::WCS.MenuPage == 0)
				{
					...
					if (LegitBotConfig::AimBot)
					{
						...
						PutSwitch("Use Alt Hotkey", 10.f, ImGui::GetFrameHeight() * 1.7, &AimControl::UseAltHotKey, false, NULL, NULL, "Use right mouse button instead of left mouse for hold mode");
						
						// Aimbot Status Indicator
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 10.f);
						if (LegitBotConfig::AimToggleMode) {
							// Toggle mode - show toggle state
							static bool aimbotToggleState = false;
							static bool lastAimbotKeyState = false;
							int activeHotKey = AimControl::UseAltHotKey ? AimControl::AltHotKey : AimControl::HotKey;
							bool currentAimbotKeyState = (GetAsyncKeyState(activeHotKey) & 0x8000) != 0;
							
							if (currentAimbotKeyState && !lastAimbotKeyState) {
								aimbotToggleState = !aimbotToggleState;
							}
							lastAimbotKeyState = currentAimbotKeyState;
							
							if (aimbotToggleState) {
								ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), "Aimbot: ENABLED (Toggle)");
							} else {
								ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "Aimbot: DISABLED (Toggle)");
							}
						} else {
							// Hold mode - show hold state
							int activeHotKey = AimControl::UseAltHotKey ? AimControl::AltHotKey : AimControl::HotKey;
							bool isHoldingAimbot = (GetAsyncKeyState(activeHotKey) & 0x8000) != 0;
							if (isHoldingAimbot) {
								ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), "Aimbot: ACTIVE (Hold)");
							} else {
								ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "Aimbot: INACTIVE (Hold)");
							}
						}
						
						// Enhanced Aimbot Controls
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 10.f);
						ImGui::TextDisabled("Enhanced Features");
						PutSwitch("Advanced Prediction", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::AdvancedPrediction);
						PutSwitch("Intelligent Targeting", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::IntelligentTargeting);
						PutSwitch("Wall Avoidance", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::WallAvoidance);
						PutSwitch("Weapon Adaptive", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::WeaponAdaptive);
						PutSwitch("Ultra Low Latency", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::UltraLowLatency);
						PutSwitch("Headshot Priority", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::HeadshotPriority);
						PutSwitch("Dynamic FOV", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::DynamicFOV);
						PutSwitch("Anti Detection", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::AntiDetection);
						
						// Advanced Settings
						if (LegitBotConfig::AdvancedPrediction) {
							static const float predTimeMin = 0.01f, predTimeMax = 0.5f;
							static const int histMin = 5, histMax = 20;
							PutSliderFloat("Prediction Time", 10.f, &LegitBotConfig::PredictionTime, &predTimeMin, &predTimeMax, "%.2f");
							PutSliderInt("History Size", 10.f, &LegitBotConfig::HistorySize, &histMin, &histMax, "%d");
						}
						
						if (LegitBotConfig::WeaponAdaptive) {
							PutSwitch("Weapon Specific Smoothing", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::WeaponSpecificSmoothing);
							if (LegitBotConfig::WeaponSpecificSmoothing) {
								static const float smoothMin = 0.1f, smoothMax = 2.0f;
								PutSliderFloat("Sniper Smooth Factor", 10.f, &LegitBotConfig::SniperSmoothFactor, &smoothMin, &smoothMax, "%.2f");
								PutSliderFloat("Rifle Smooth Factor", 10.f, &LegitBotConfig::RifleSmoothFactor, &smoothMin, &smoothMax, "%.2f");
								PutSliderFloat("Pistol Smooth Factor", 10.f, &LegitBotConfig::PistolSmoothFactor, &smoothMin, &smoothMax, "%.2f");
								PutSliderFloat("SMG Smooth Factor", 10.f, &LegitBotConfig::SMGSmoothFactor, &smoothMin, &smoothMax, "%.2f");
							}
						}
						
						if (LegitBotConfig::AntiDetection) {
							PutSwitch("Advanced Humanization", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::AdvancedHumanization);
							static const float jitterMin = 0.0f, jitterMax = 1.0f;
							static const float reactionMin = 0.0f, reactionMax = 0.5f;
							PutSliderFloat("Micro Jitter Strength", 10.f, &LegitBotConfig::MicroJitterStrength, &jitterMin, &jitterMax, "%.2f");
							PutSliderFloat("Reaction Time Variation", 10.f, &LegitBotConfig::ReactionTimeVariation, &reactionMin, &reactionMax, "%.2f");
							PutSwitch("Curved Movement", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::CurvedMovement);
							PutSwitch("Velocity Based Smoothing", 10.f, ImGui::GetFrameHeight() * 1.7, &LegitBotConfig::VelocityBasedSmoothing);
						}
						...
						// Enhanced TriggerBot Controls
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 10.f);
						ImGui::TextDisabled("Enhanced TriggerBot");
						PutSwitch("Ultra Fast Mode", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::UltraFastMode);
						PutSwitch("Predictive Trigger", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::PredictiveTrigger);
						PutSwitch("Weapon Adaptive", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::WeaponAdaptive);
						PutSwitch("Anti Detection", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::AntiDetection);
						PutSwitch("Smart Timing", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::SmartTiming);
						PutSwitch("Velocity Prediction", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::VelocityPrediction);
						PutSwitch("Headshot Priority", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::HeadshotPriority);
						PutSwitch("Multi Target", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::MultiTarget);
						
						// Hold/Toggle Controls
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 10.f);
						ImGui::TextDisabled("TriggerBot Mode");
						PutSwitch("Toggle Mode", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::ToggleMode, false, NULL, NULL, "Toggle: Press key to enable/disable. Hold: Keep key pressed.");
						
						// TriggerBot Status Indicator
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 10.f);
						if (TriggerBot::ToggleMode) {
							if (TriggerBot::ToggleState) {
								ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), "TriggerBot: ENABLED (Toggle)");
							} else {
								ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "TriggerBot: DISABLED (Toggle)");
							}
						} else {
							bool isHolding = (GetAsyncKeyState(TriggerBot::HotKey) & 0x8000) != 0;
							if (isHolding) {
								ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), "TriggerBot: ACTIVE (Hold)");
							} else {
								ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "TriggerBot: INACTIVE (Hold)");
							}
						}
						
						// Advanced TriggerBot Settings
						if (TriggerBot::UltraFastMode || TriggerBot::PredictiveTrigger) {
							static const float confMin = 0.0f, confMax = 1.0f;
							static const int histMin = 3, histMax = 10;
							static const float predMin = 0.01f, predMax = 0.2f;
							PutSliderFloat("Confidence Threshold", 10.f, &TriggerBot::ConfidenceThreshold, &confMin, &confMax, "%.2f");
							PutSliderInt("History Size", 10.f, &TriggerBot::HistorySize, &histMin, &histMax, "%d");
							PutSliderFloat("Prediction Time", 10.f, &TriggerBot::PredictionTime, &predMin, &predMax, "%.3f");
						}
						
						if (TriggerBot::AntiDetection) {
							PutSwitch("Humanized Timing", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::HumanizedTiming);
							PutSwitch("Random Delay", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::RandomDelay);
							static const float timingMin = 0.0f, timingMax = 0.5f;
							static const float delayMin = 0.1f, delayMax = 5.0f;
							PutSliderFloat("Timing Variation", 10.f, &TriggerBot::TimingVariation, &timingMin, &timingMax, "%.2f");
							PutSliderFloat("Min Delay", 10.f, &TriggerBot::MinDelay, &delayMin, &delayMax, "%.1f");
							PutSliderFloat("Max Delay", 10.f, &TriggerBot::MaxDelay, &delayMin, &delayMax, "%.1f");
						}
						
						if (TriggerBot::WeaponAdaptive) {
							PutSwitch("Batch Processing", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::BatchProcessing);
							PutSwitch("Memory Optimized", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::MemoryOptimized);
							PutSwitch("Frame Perfect", 10.f, ImGui::GetFrameHeight() * 1.7, &TriggerBot::FramePerfect);
						}
