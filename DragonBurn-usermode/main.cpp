...
https://github.com/ByteCorum/DragonBurn
	...
	{
		Log::PreviousLine();
		Log::Warning("Kernel mode driver disabled. Continuing in user-mode");
	}

	std::cout << '\n';
	bool preStart = false;
	while (memoryManager.GetProcessID(L"cs2.exe") == 0 && memoryManager.GetProcessID_UserMode(L"cs2.exe") == 0)
	{
		Log::PreviousLine();
	...
	DWORD targetPid = memoryManager.GetProcessID(L"cs2.exe");
	if (targetPid == 0)
	{
		targetPid = memoryManager.GetProcessID_UserMode(L"cs2.exe");
	}

	bool attached = false;
	if (targetPid != 0 && memoryManager.ConnectDriver(L"\\.\DragonBurn-kmd"))
	{
		attached = memoryManager.Attach(targetPid);
	}

	if (!attached && targetPid != 0)
	{
		attached = memoryManager.Attach_UserMode(targetPid);
	}

	if (!attached)
	{
		Log::PreviousLine();
		Log::Error("Failed to attach to CS2");
		return;
	}
