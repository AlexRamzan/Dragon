...
{
	this->Address.ClientDLL = memoryManager.GetModuleBase(L"client.dll");
	if (this->Address.ClientDLL == 0)
	{
		this->Address.ClientDLL = memoryManager.GetModuleBase_UserMode(L"client.dll");
	}

	this->Address.ServerDLL = memoryManager.GetModuleBase(L"server.dll");
	if (this->Address.ServerDLL == 0)
	{
		this->Address.ServerDLL = memoryManager.GetModuleBase_UserMode(L"server.dll");
	}
