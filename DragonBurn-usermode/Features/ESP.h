...
{"c4", {13.0f, 13.0f, 0.0f, 0.0f}},
	...
	// Lightweight rect cache to reduce flicker and recover when data blips
	// Keyed by entity pawn address; auto-clears every 30s via watchdog
	static std::unordered_map<DWORD64, ImVec4> s_lastRectByPawn;
	static std::unordered_map<DWORD64, ULONGLONG> s_lastSeenTickByPawn;
	static ULONGLONG s_lastGlobalResetTick = 0;

	inline void WatchdogTick()
	{
		ULONGLONG now = GetTickCount64();
		if (s_lastGlobalResetTick == 0) s_lastGlobalResetTick = now;
		if (now - s_lastGlobalResetTick >= 30000ULL) // reset every 30s
		{
			s_lastRectByPawn.clear();
			s_lastSeenTickByPawn.clear();
			s_lastGlobalResetTick = now;
		}
	}
	...
	{
		// Watchdog ensures cache is refreshed periodically to self-heal
		WatchdogTick();

		if (Entity.Pawn.Pos == Vec3(0, 0, 0))
			return;

		// If incoming Rect seems invalid, recompute; else fall back to last known
		bool rectInvalid = (Rect.z <= 0.5f || Rect.w <= 0.5f);
		if (rectInvalid)
		{
			ImVec4 recomputed = Render::Get2DBox(Entity);
			bool recomputedOk = !(recomputed.z <= 0.5f || recomputed.w <= 0.5f);
			if (recomputedOk)
			{
				Rect = recomputed;
			}
			...
			{
				DWORD64 key = Entity.Pawn.Address;
				auto it = s_lastRectByPawn.find(key);
				if (it != s_lastRectByPawn.end())
				{
					Rect = it->second; // use last known good rect to avoid disappearance
				}
			}
		}

		// Update cache if we have a sane rect
		if (!(Rect.z <= 0.5f || Rect.w <= 0.5f))
		{
			s_lastRectByPawn[Entity.Pawn.Address] = Rect;
			s_lastSeenTickByPawn[Entity.Pawn.Address] = GetTickCount64();
		}
